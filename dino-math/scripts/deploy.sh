...
# Dino Math Runner — SAM build + deploy script
...
sam build \
  --template-file "$GAME_ROOT/infra/template.yaml" \
  --build-dir "$GAME_ROOT/.aws-sam/build"

echo "==> Deploying SAM stack: $STACK_NAME"
sam deploy \
  --template-file "$GAME_ROOT/.aws-sam/build/template.yaml" \
